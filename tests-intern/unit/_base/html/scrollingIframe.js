...
function runScrollingTest(resultNode) {
	...
	setTimeout(function () {
		...
		setTimeout(function () {
			...
			if (resultNode.resultReady) {
				resultNode.resultReady();
			}
		}, 100);
	}, 100);
}
...
function genScrollingTestNodes(hScroll, vScroll, large) {
	document.write(
		...
		(large ? ((hScroll ? '<div style="float:left;position:relative;width:600px;">&nbsp;</div>' : '') +
...
function genScrollingTestBody() {
	...
	document.write('<body style="height:100%;margin:0;padding:0;border:0;background-color:white;overflow-x:' +
		(options.horz ? (isQuirks ? scroll : '') : 'hidden') + ';overflow-y:' +
		(isQuirks ? (options.vert ? scroll : 'hidden') : '') + ';">');
	document.write('<div id="clientWidth"><center>' + (isQuirks?'quirks ':'strict ') + (options.horz?'horiz ':'') +
		(options.vert? 'vert ' : '') + (options.large ? 'scrolling ' : '') + options.dir + '</center></div>');
